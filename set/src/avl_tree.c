...
void iterator_avltree(avltree *root, struct iterator **iter) {
	if (root == NULL) return;
	if (*root != NULL) {
		iterator_avltree(&((*root)->left), iter);
		struct iterator* node;
		node = (struct iterator*) malloc(sizeof(struct iterator));
		node->value = (*root)->value;
		node->next = *iter;
		*iter = node;
		
		iterator_avltree(&((*root)->right), iter);
	}
}
